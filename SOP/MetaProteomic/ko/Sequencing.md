# Sequencing
## Biological Samples
- Fecal sample 1 g당 500ul RNAlater (Ambion)를 처리하여 -80℃에 보관한다. 
...
- MP FastPrep 6.0m/s 40초 두번 진행한다.
- 12,000 rpm에서 3분간 원심분리 후, 상층액 500 μL를 새 e-tube 로 옮긴다.
- 3M sodium acetate 50 μL, 1mL 100% ethanol을 처리한다.
- Ice (또는 4℃)에서 30분 반응시킨다.
- 12,000 rpm 4℃에서 30분간 원심분리 후, 상층액을 제거한다.
- 100 μL Stock solution B로 재현탁한다.
- 1/50로 희석한 RNA를 Qubit으로 RNA 농도를 측정한다.
- MNase와 2시간 반응시킨다. 
| Total  | 200μL |
| ---| --- |
| Lysate | 80 μg |
...
- 0.5M EGTA 2.5 μL 처리하여 반응을 중단시킨다
- Sephacryl S400 MicroSpin column을 준비하여 column 안의 resin을 재현탁한다. 
- Column을 600 rpm 에서 1 분간 원심분리 후 통과액을 제거한다.
- Column에 Stock solution C 500 μL를 넣고 600 rpm 4℃에서 1분간 원심분리 후 통과액을 제거한다. (두번 반복)
- Column에 Stock solution C 500 μL를 넣고 600 rpm 4℃에서 4분간 원심분리 후 통과액을 제거 후 새로운 e-tube에 column을 넣는다.
- 각 column에 100 μL의 10)의 MNase 반응액을 처리하고 600 rpm 2분간 원심분리 후 통과액을 얻는다.
- 통과액에 Qiazol 700 μL를 처리하고 vortexing후 상온에서 5분 반응시킨다.
- Chloroform 140 μL를 넣고 15초 흔들어준 후 상온에서 3분 반응시킨다.
- 12,000 rpm 4℃에서 15분간 원심분리 후 상층액을 새 e-tube에 옮겨준다.
- 상층액의 1.5배의 양(약 525 μL)의 100% ethanol을 넣고 pipetting으로 섞어준다. *
- RNeasy mini column에 *단계의 용액을 700 μL를 넣고 12,000 rpm에서 15초 원심분리 후 통과액을 버린다.
- 남은 *단계의 용액으로 이전 단계를 한 번 더 진행한다.
- RWT buffer 700 μL를 column에 처리하고 12,000 rpm에서 15초간 원심분리 후 통과액을 버린다.
- RPE buffer 500 μL를 column에 처리하고 12,000 rpm에서 15초간 원심분리 후 통과액을 버린다.
- RPE buffer 500 μL를 column에 처리하고 12,000 rpm에서 2분간 원심분리 후 column을 새 e-tube에 옮긴다.
- RNase-free water 30-50μL 를 column의 membrane에 처리하여 12,000 rpm에서 1분간 원심분리 후 ribosome-bound mRNA를 얻는다
...
- 37℃에 20-30분 반응시켜준다.
- 0.1배 volume의 DNase inactivation reagent를 처리한다.
- 상온에서 5분 반응시켜준다
